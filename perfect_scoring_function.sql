-- Perbaikan fungsi submit_assignment untuk memastikan perhitungan skor akurat sesuai jawaban benar
...
BEGIN
  ...
  LOOP
    ...
    -- Verify correctness
    v_is_correct := FALSE;
    IF v_question_correct_option IS NOT NULL AND v_answer.selected_option_id IS NOT NULL THEN
      IF v_question_correct_option = v_answer.selected_option_id THEN
        v_is_correct := TRUE;
    ...
    -- Optionally store result details (for debugging or later retrieval)
    v_result_details := v_result_details || jsonb_build_object(
      ...
      'correct_option_id', v_question_correct_option,
